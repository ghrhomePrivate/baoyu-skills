...
`--style X` 展开为色板 + 渲染的组合。用户可以覆盖任一维度。
...
- `--style blueprint --rendering hand-drawn` = cool 色板搭配 hand-drawn 渲染
- `--style elegant --palette warm` = warm 色板搭配 hand-drawn 渲染

显式的 `--palette`/`--rendering` 标志始终覆盖预设值。
